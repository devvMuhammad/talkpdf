...
export function FileProgressCards({ 
  ...
  // Show the component if uploading or indexing
  if (!isUploading && !isIndexing) return null
  ...
  return (
    <div className={cn("space-y-3", className)}>
      ...
      {/* Indexing Card - only show when indexing */}
      {isIndexing && (
        <div className={cn(
          "flex items-center gap-4 p-4 rounded-lg border transition-all duration-500",
          ...
          <div className="flex-1 min-w-0">
            <div className="font-medium text-sm text-purple-800 dark:text-purple-200 transition-colors">
              Processing & Indexing
            </div>
            <div className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">
              Extracting text • Creating embeddings • Vector storage
            </div>
          </div>
        </div>
      )}
